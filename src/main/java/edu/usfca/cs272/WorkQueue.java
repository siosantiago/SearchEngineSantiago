...
public class WorkQueue {
	...
	/**Pending variable to see if continue*/
	private int pending;
	...
	public synchronized void finish() {
		log.debug("Entered finish");
		while(pending > 0) {
			log.debug("Is it finish");
			try {
				this.wait();
		...
		log.debug("Is task empty: " + tasks.isEmpty());
	...
	private class Worker extends Thread {
		...
		public Worker() {
			...
			log.trace("Worker"  + getName() +" was initiazized");
		}
				...
				while (true) {
					synchronized (tasks) {
						...
						if (shutdown) {
							...
							log.debug("Worker {} found {} tasks...", getName(), tasks.size());
					...
					try {
						...
						task.run();
						decrementPending();
						log.trace("Work queue worker finsished work.");
					}
